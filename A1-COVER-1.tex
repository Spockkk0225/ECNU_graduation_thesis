\pagestyle{empty}
\setlength{\baselineskip}{25pt}  %%正文设为25磅行间距
\vspace{-2.0cm}
\noindent{{\zihao{4} {\large 2025} 届硕士学术学位研究生学位论文}}\\
\vspace{-0.8cm}
\begin{center}
	\hspace*{-1cm}
	\renewcommand\arraystretch{1.5}
	\begin{tabular}{l}
		\noindent{{\zihao{4} 分类号：\underline{~~~\qquad\qquad\qquad\qquad}}}   \\
		\noindent{{\zihao{4} 密~~~~级：\underline{~~~\qquad\qquad\qquad\qquad}}} \\
	\end{tabular}
	\hspace{1.6cm}
	\renewcommand\arraystretch{1.5}
	\begin{tabular}{r}
		\noindent{{\zihao{4} 学校代码：\underline{~~~\qquad10269\qquad~~~}}}   \\  %10269
		\noindent{{\zihao{4} 学~~~~~~~~号：\underline{~~~~
		% xxxxxxxxxx
		\qquad \qquad \ \ \ \ \ \ \
		~~~~}}} \\
	\end{tabular}
	\hspace*{-1cm}
\end{center}


\vskip 1.5cm

\begin{center}
	% \hskip 0.5cm
	% \scalebox{1.0}{\includegraphics[width=2.5cm]{fig/ecnu_logo.png}}
	% \scalebox{1.0}{\includegraphics[width=10.5cm]{fig/ecnu_title.png}}
	\scalebox{1.0}{\includegraphics[width=13cm]{fig/title.png}}
	\vskip 0.1cm
		{{{\xiaosi East China Normal University}}}\\
		% \vskip 0.2cm
		{\textbf{\xiaosi 硕士学术学位论文}}\\
		% \vskip 0.2cm
		{\textbf{{\xiaosi Master's Degree Thesis (Professional)}}}\\
\end{center}


\vskip 0.5cm

\begin{center}
	{\yihao \bf {基于核密度估计的时空路网}}

	{\yihao \bf {数据可视化研究}}
	% {\erhao \bf 论文题目：\underline{融合知识的预训练语言模}}
	% \\
	% {\erhao \bf ~~~~\underline{型相关技术研究}}
\end{center}

\vskip 1.0cm

\begin{center}
	\renewcommand\arraystretch{1.5}
	\begin{tabular}{l}
		{\sihao \bf 院\qquad\qquad 系:} \\
		{\sihao \bf 学~~~位~~类~~~别:}      \\
		{\sihao \bf 学~~~位~~领~~~域:}      \\
		{\sihao \bf 指~~~导~~教~~~师:}      \\
		{\sihao \bf 学~位~申~请~人:}
	\end{tabular}
	\begin{tabular}c
		% 盲审需要注释下面一些信息，注意页面上的学号是否需要注释

		{\sihao \bf  \qquad 软件工程学院 \qquad} \\
		\hline {\sihao \bf 学术学位硕士}                  \\
		\hline {\sihao \bf 软件工程}                 \\
		\hline {\sihao \bf }                     \\
		\hline {\sihao \bf }                     \\

		% {\sihao \bf  \qquad ~~~~~~~~~~~~~~~~~~~~ \qquad} \\
		% \hline {\sihao}                                  \\
		% \hline {\sihao}                                  \\
		% \hline {\sihao}                                  \\
		% \hline {\sihao}                                  \\

		\hline
	\end{tabular}
\end{center}

\vskip 2.0cm

\begin{center}
	{\sihao 2025年4月1日}
\end{center}

\cleardoublepage